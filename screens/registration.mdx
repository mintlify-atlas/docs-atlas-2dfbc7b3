...
<Tip>
  For persistent credential storage across app sessions, see [data/local-database](/data/local-database) for guidance on implementing a SQLite-backed solution using Android's `SQLiteOpenHelper` or Room.
</Tip>
